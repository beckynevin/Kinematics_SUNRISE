...
PRO run_kinemetry_on_multiple_galaxies, vers, id, name, view


string_underlined = STRJOIN(STRSPLIT(STRING(id),'-', /EXTRACT), '_')

;# First check to see if it needs to run on a specific viewpoint
file_check='kinemetry_solution/vcirc_model_'+STRING(vers)+'_'+STRING(name)+'_'+STRING(id)+'_'+STRING(view)+'.txt'
file_check = STRCOMPRESS(file_check, /REMOVE_ALL)
PRINT, 'checking this file', file_check
cont = FILE_TEST(file_check)

IF (cont EQ 1) THEN BEGIN
	PRINT, cont, 'already exists'
	RETURN
	END

file = 'kinemetry_input_txt/kinemetry_input_'+STRING(vers)+'_'+STRING(name)+'_'+STRING(id)+'_'+STRING(view)+'.txt'
file = STRCOMPRESS(file, /REMOVE_ALL)

check = FILE_TEST(file)

IF (check EQ 0) THEN BEGIN
	PRINT, check, 'not ready to run yet'
	RETURN
	END
...
file_out='kinemetry_solution/vcirc_model_'+STRING(vers)+'_'+STRING(name)+'_'+STRING(id)+'_'+STRING(view)+'.txt'
file_out = STRCOMPRESS(file_out, /REMOVE_ALL)
writecol, file_out, xbin, ybin, velbin, er_velbin, velCirc, velKin, fmt='(f,f,f,f,x,f,x,f)'
file_out_2 = 'kinemetry_solution/kinemetry_params_'+STRING(vers)+'_'+STRING(name)+'_'+STRING(id)+'_'+STRING(view)+'.txt'
